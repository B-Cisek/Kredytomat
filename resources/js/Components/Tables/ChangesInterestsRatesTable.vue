...
const defaultResult = ref({
  interest: props.credit.margin + props.credit.wibor,
  installment: props.schedule[0][4],
  creditInterests: props.schedule[0][2],
  creditCapital: props.schedule[0][3],
...
function calc() {
  for (let i = 0; i < 22; i++) {
    ...
    result.value.push({
      interest: {
        value: props.credit.margin + props.credit.wibor + interestRateChanges[i],
        change: props.credit.margin + props.credit.wibor + interestRateChanges[i] - defaultResult.value.interest
      },
      installment: {
        value: newSchedule[0][4],
        change: changeCalc(defaultResult.value.installment, newSchedule[0][4])
      },
      creditInterests: {
        value: newSchedule[0][2],
        change: changeCalc(defaultResult.value.creditInterests, newSchedule[0][2])
      },
      creditCapital: {
        value: newSchedule[0][3],
        change: changeCalc(defaultResult.value.creditCapital, newSchedule[0][3])
      },
      creditInterest10years: {
        value: getPaidInterestToIndex(newSchedule, 12 * 10),
        change: changeCalc(getPaidInterestToIndex(props.schedule, 12 * 10), getPaidInterestToIndex(newSchedule, 12 * 10))
      },
      creditCapital10years: {
        value: getPaidCapitalToIndex(newSchedule, 12 * 10),
        change: changeCalc(getPaidCapitalToIndex(props.schedule, 12 * 10), getPaidCapitalToIndex(newSchedule, 12 * 10))
      },
      totalCreditInterest: {
